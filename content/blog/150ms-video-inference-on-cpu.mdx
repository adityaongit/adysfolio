...
<Mermaid code={`
flowchart TD
    A[Video Input<br/>Webcam/RTSP/File] --> B[Gate 1: Motion Detection<br/>MOG2 Background Subtraction<br/>~5ms per frame]
    B -->|Motion Detected| C[Gate 2: Scene Change<br/>HSV Histogram + SSIM<br/>~3ms]
    B -->|No Motion| Z[Frame Discarded]
    C -->|Scene Changed OR Motion Spike| D[Gate 3: Object Detection<br/>YOLOv5n + IOU Tracking<br/>8-15ms]
    C -->|No Significant Change| Z
    D -->|New Object Confirmed| E[Queue for LLM<br/>Background Thread]
    D -->|No New Objects| Z
    E --> F[LLM Analysis<br/>~100ms async]
    ...
    style Z fill:#f8f9fa,stroke:#6c757d,stroke-dasharray: 5 5
`} />
...
<Mermaid code={`
gantt
    title Frame Processing Latency Breakdown (Triggered Path)
    dateFormat s
    axisFormat %Lms

    section Frame Processing
    Capture + Preprocessing     :0, 5
    Gate 1: Motion (MOG2)     :5, 10
    Gate 2: Scene Change        :10, 13
    Gate 3: YOLOv5n (MPS)    :13, 28
    Queue Frame                 :28, 28
    
    section Async Processing
    LLM Analysis (Background)    :28, 128
    
    section Total
    Total Latency               :128, 128
`} />
